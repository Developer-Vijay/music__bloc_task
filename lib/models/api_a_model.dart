...
//
//     final apiAModel = apiAModelFromJson(jsonString);

import 'dart:convert';
import 'package:http/http.dart' as http;

ApiAModel apiAModelFromJson(String str) =>
    ApiAModel.fromJson(json.decode(str));

String apiAModelToJson(ApiAModel data) => json.encode(data.toJson());
...
class ApiAModel {
  ...
  factory ApiAModel.fromJson(Map<String, dynamic> json) => ApiAModel(
...
class PrimaryGenres {
  ...
  Map<String, dynamic> toJson() => {
        "music_genre_list": List<dynamic>.from(
            musicGenreList!.map((x) => x.toJson())),
      };
}
...
class TrackNameTranslationList {
  ...
  factory TrackNameTranslationList.fromJson(
          Map<String, dynamic> json) =>
      TrackNameTranslationList(
        trackNameTranslation: TrackNameTranslation.fromJson(
...
class ApiAService {
  ...
  Future<ApiAModel> getApiA() async {
    var url = Uri.parse(
        "https://api.musixmatch.com/ws/1.1/chart.tracks.get?apikey=0ff5ec0ac765a5e82c4967f2323553c2");

    final response = await cleient.get(url);

    final activity = apiAModelFromJson(response.body);
    return activity;
  }
}
